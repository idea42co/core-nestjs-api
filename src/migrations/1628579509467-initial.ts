...
export class initial1628579509467 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "items" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "test" varchar NOT NULL, "organizationId" integer)`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE "temporary_items" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "test" varchar NOT NULL, "organizationId" integer, CONSTRAINT "FK_68bb72ce92f1bcab12c2019f99d" FOREIGN KEY ("organizationId") REFERENCES "organizations" ("id") ON DELETE NO ACTION ON UPDATE NO ACTION)`,
    );
    await queryRunner.query(
      `INSERT INTO "temporary_items"("id", "test", "organizationId") SELECT "id", "test", "organizationId" FROM "items"`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `INSERT INTO "items"("id", "test", "organizationId") SELECT "id", "test", "organizationId" FROM "temporary_items"`,
    );
